      ...
      <div class="container legal-body">
        @if (loading()) {
          ...
          <div class="empty-state">
            <p>
              Our terms and conditions are being prepared. Please check back
              soon.
            </p>
...
export class TermsComponent implements OnInit {
  ...
  ngOnInit() {
    this.api.get<any>("/legal").subscribe({
      next: (data) => {
        this.content.set(data.termsAndConditions || "");
        if (data.termsUpdatedAt) {
          this.updatedAt.set(
            new Date(data.termsUpdatedAt).toLocaleDateString("en-GB", {
              day: "numeric",
              month: "long",
              year: "numeric",
            }),
