...
@Component({
    ...
    <nav class="navbar" [class.scrolled]="scrolled()" [class.menu-open]="menuOpen()">
      <div class="nav-container">
        ...
        <div class="nav-actions">
          <button class="btn-icon" (click)="themeService.toggle()" title="Toggle theme">
          ...
          @if (auth.isLoggedIn()) {
            ...
            <div class="user-menu">
              <button class="user-btn" (click)="toggleUserMenu()">
                <span class="user-avatar">{{ auth.user()?.firstName?.[0] }}</span>
              </button>
              @if (userMenuOpen()) {
                <div class="user-dropdown">
                  <a routerLink="/orders" (click)="closeUserMenu()">My Orders</a>
                  <a routerLink="/favorites" (click)="closeUserMenu()">Favorites</a>
                  <hr>
            ...
            <a routerLink="/auth/login" class="btn btn-sm btn-outline-accent">Sign In</a>
          }

          <button class="hamburger" (click)="menuOpen.set(!menuOpen())" [class.active]="menuOpen()">
      ...
      @if (menuOpen()) {
        <div class="mobile-menu">
          ...
          @if (auth.isLoggedIn()) {
            ...
            <a routerLink="/favorites" (click)="menuOpen.set(false)">Favorites</a>
            <a routerLink="/cart" (click)="menuOpen.set(false)">Cart ({{ cart.itemCount() }})</a>
            <button (click)="logout()">Sign Out</button>
          } @else {
            <a routerLink="/auth/login" (click)="menuOpen.set(false)">Sign In</a>
            <a routerLink="/auth/register" (click)="menuOpen.set(false)">Register</a>
          }
        </div>
      }
    </nav>
  ...
  styles: [`
    ...
    .nav-links a {
      font-size: 0.875rem; font-weight: 400; letter-spacing: 0.08em; text-transform: uppercase;
      color: var(--color-text-secondary); text-decoration: none; position: relative;
    ...
    .btn-icon {
      ...
      cursor: pointer; color: var(--color-text); transition: all 0.3s;
      text-decoration: none; font-size: 1rem; position: relative;
    }
    .btn-icon:hover { background: var(--color-surface-2); border-color: var(--color-accent); }
    ...
    .user-btn {
      width: 40px; height: 40px; border-radius: 50%;
      background: var(--color-accent); color: white;
      border: none; cursor: pointer; font-weight: 600; font-size: 0.9375rem;
      font-family: var(--font-display);
    }
    .user-dropdown {
      position: absolute; top: calc(100% + 8px); right: 0;
      background: var(--color-surface); border: 1px solid var(--color-border);
      border-radius: var(--radius-sm); padding: 8px 0; min-width: 160px;
      box-shadow: var(--shadow-lg); z-index: 10;
    }
    .user-dropdown a, .user-dropdown button {
      display: block; width: 100%; padding: 10px 16px;
      font-size: 0.875rem; color: var(--color-text); text-decoration: none;
      background: none; border: none; cursor: pointer; text-align: left;
      transition: background 0.2s;
    }
    .user-dropdown a:hover, .user-dropdown button:hover { background: var(--color-surface-2); }
    ...
    .mobile-menu a, .mobile-menu button {
      padding: 14px 0; font-size: 1.125rem;
