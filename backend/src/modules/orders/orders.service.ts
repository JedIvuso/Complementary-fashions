...
export class OrdersService {
  ...
  async createOrder(userId: string, deliveryDetails: any) {
    ...
    const subtotal = cartItems.reduce((sum, item) => {
      const price = Number(item.product.price) + Number(item.variant?.additionalPrice || 0);
    ...
    const orderItems = cartItems.map((item) =>
      this.orderItemsRepository.create({
        ...
        unitPrice: Number(item.product.price) + Number(item.variant?.additionalPrice || 0),
        quantity: item.quantity,
        totalPrice: (Number(item.product.price) + Number(item.variant?.additionalPrice || 0)) * item.quantity,
  ...
  async findOne(id: string) {
    const order = await this.ordersRepository.findOne({
      where: { id },
      relations: ['user', 'items', 'items.product', 'items.product.images', 'payments'],
    });
  ...
  async getDashboardStats() {
    ...
    const paid = await this.ordersRepository.count({ where: { status: OrderStatus.PAID } });
