    ...
    <div class="dashboard">
      <div class="page-title-bar">
        ...
        <div class="header-actions" style="display:flex;gap:10px">
          <a [href]="publicUrl" target="_blank" class="btn btn-ghost btn-sm">
            🌐 View Store
          </a>
        ...
        <div class="stats-grid">
          ...
          <div class="stat-card">
            ...
            <div class="stat-value">
              KSh {{ stats?.totalRevenue | number: "1.0-0" }}
            </div>
            ...
            <table>
              ...
              <tbody>
                @for (order of recentOrders(); track order.id) {
                  <tr>
                    <td>
                      <span style="font-weight:500;color:var(--color-accent)">{{
                        order.orderNumber
                      }}</span>
                    </td>
                    <td>
            ...
            @for (month of monthlySales(); track month.month) {
              <div class="month-row">
                <span class="month-label">{{
                  month.month | date: "MMM yyyy"
                }}</span>
                <div class="month-bar-wrap">
                  <div
                    class="month-bar"
                    [style.width]="getBarWidth(month.revenue)"
                  ></div>
                </div>
                <span class="month-val"
                  >KSh {{ month.revenue | number: "1.0-0" }}</span
                >
              </div>
            }
            @if (!monthlySales().length && !loading()) {
              <p
                style="text-align:center;color:var(--color-text-muted);padding:32px 0"
              >
                No sales data yet
              </p>
            }
...
export class DashboardComponent implements OnInit {
  ...
  admin: any;
  publicUrl =
    environment.publicUrl || "https://complementary-fashions.vercel.app";
    ...
    this.api.get<any>("/admin/dashboard").subscribe({
      next: (data) => {
        ...
        this.maxRevenue = Math.max(
          ...(data.monthlySales || []).map((m: any) => Number(m.revenue)),
          1,
        );
